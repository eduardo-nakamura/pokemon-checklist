  ...
  return (
    <tr
      ...
      className={`group transition-colors cursor-pointer select-none border-b last:border-0 ${
        isCaught
          ? isDarkMode ? 'bg-green-500/10 border-slate-700/50' : 'bg-green-50/60 border-slate-200'
          : isDarkMode ? 'hover:bg-slate-700/20 border-slate-700/50' : 'hover:bg-slate-50 border-slate-200'
      }`}
    >
      <td className={`p-4 text-xs font-mono ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
      ...
      <td className='p-4 text-center'>
        <div className='flex flex-col items-center gap-1'>
          <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${
            pokemon.captureRate > 200 ? 'bg-green-100 text-green-700' : 
            pokemon.captureRate > 100 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'
          }`}>
