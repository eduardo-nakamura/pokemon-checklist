...
interface PokeApiSpecies {
  id: number // Adicionado para o ID da espécie
  names: { language: { name: string }; name: string }[]
  capture_rate: number
  varieties: { is_default: boolean; pokemon: { name: string; url: string } }[] // Tipagem das variedades
}
...
export const fetchPokemonData = async (
  ...
  const pokemonPromises = data.pokemon_entries.map(
    async (entry: PokeApiPokedexEntry) => {
      try {
        // 1. Primeiro buscamos os dados da espécie para verificar variedades
        ...
        // 2. Lógica para encontrar a Forma Regional correta
        // Procuramos nas variedades se existe uma que contenha o nome da região (ex: 'alola')
        const regionalVariety = speciesData.varieties.find(
          v => isSM && v.pokemon.name.includes('-alola')
        )

        // Se acharmos a forma regional, usamos a URL dela, senão a padrão
        const targetPokemonUrl = regionalVariety
          ? regionalVariety.pokemon.url
          : `https://pokeapi.co/api/v2/pokemon/${pokemonId}`

        // 3. Buscamos detalhes e encontros da forma CORRETA
        ...
        // ... resto da tradução do nome
        let translatedName = entry.pokemon_species.name
        if (lang === 'ja') {
          // 3. Tipagem correta no nome japonês (Resolve o erro de 'any' da linha 69)
          translatedName =
        ...
        return {
          id: pokemonId, // Mantemos o ID da espécie para o checklist ser consistente
          name:
            lang === 'ja'
              ? translatedName
              : translatedName.charAt(0).toUpperCase() +
                translatedName.slice(1),
          // Usamos o sprite da detailData (que agora será a de Alola se regionalVariety existir)
          sprite:
